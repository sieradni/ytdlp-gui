...
| D54 | Playlist progress counting | Playlist jobs show `items done / total` (D33): `total` comes from yt-dlp's `[download] Downloading item N of M` line (authoritative when printed; null = single video or old yt-dlp); `done` is counted from item-start lines, which makes it **monotonic and archive-skips count as processed** — a resilient lower bound rather than a possibly-wrong exact number. Final counts flush to the db at job completion so the throttled progress writes can't lose the last item. | M4, closing the M3 review flag |
| D55 | Migration is one-shot | The v1 migration runs once, gated by a `migrated_from_v1` settings flag. Re-running it on every launch would clobber post-migration v2 settings changes with stale v1 values. The discovered v1 binaries are **offered** (wizard offer, D40) — never auto-adopted; the archive import is in-place (D43) and history seeding is idempotent, so it is safe to leave the v1 files where they are. | M4 implementation |
...
| `ytdlp_path`, `ffmpeg_path` | offered as custom binaries in first-run wizard |
| `downloaded.txt` beside old exe | imported: archive path pointed at it (or copied into app-data); ids also seeded into history DB |
