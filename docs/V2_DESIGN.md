...
| D38 | Cookies UX | Cookie source = dropdown per advanced panel: none / from browser (list) / cookies.txt file picker. ~~Never persisted~~ — amended by D86: the cookie SOURCE is a composer preference and persists with the other advanced options; the cookie CONTENTS are still never stored by the app (yt-dlp reads them from the browser/file at run time). | "Cookies" alone was too vague to implement |
...
| D85 | The queue tells the truth; home edits persist; the list is manageable | Alpha.9 findings, one investigation each. (1) **The stuck "fetching" row was a rendering trap, not a stuck engine**: a video whose title YouTube serves EMPTY made the title fallback (`fetching… <url>`) render for the entire job life — it read as a status line that never changed, and the finished state (verified: db row done, history written with final path) never repainted because the terminal job:update was lost (webview stall) and nothing else asked the frontend to re-pull. Four layers fixed: the fallback text is a real state (only while fetching, never `fetching… <url>`); the done path now fires queue:changed like every other terminal path; the frontend re-pulls queue_list on every queue:changed — a lost event can only delay truth, never freeze it. (2) **Home destination is a home setting** — editing/browsing it saves to settings immediately (same store as the settings page); settings stays the default. (3) **Queue order defaults to newest-first** with active jobs pinned above finished; the # header flips newest/oldest (the old cycle had a no-op asc/desc on _order — same order both ways). (4) **Clear all** joins clear done: one click removes every non-active row (failures, stops, done; queued too — active downloads are refused by the engine and filtered client-side). | alpha.9 user feedback |
| D86 | Unicode media works end-to-end; composer options persist | Alpha.9 finding, investigated with a live reproduction. The "missing" Japanese title was never a naming problem — the file on disk is named via the windows wide-char API and was always correct (`キミエモーション [KD1DE1dalok].opus`). The real defect is in the PIPE: yt-dlp (python) writes stdout/stderr in the legacy console codepage (cp1252), where non-ansi text cannot pass. Reproduced hard failure: a Japanese-titled video's destination line made python abort the whole download mid-stream with `OSError: [Errno 22] Invalid argument` (the .part file was left behind); softer failures include mangled titles/paths in the app and dropped error lines (D83). Fix: every spawned child gets `PYTHONUTF8=1` + `PYTHONIOENCODING=utf-8` — the same video then downloads clean in the app, and history records the full unicode path. Belt-and-braces: a printed final path that doesn't exist on disk is discarded (recovery scan still applies), so a mangled path can never masquerade as the real file. Composer persistence: dlType/format/cover/playlist/cookies/sponsorblock/subs/extra-args now survive relaunch via `settings.composeOpts` (seeded when settings load; v1 migration yields to user-persisted options; `overwrite` stays per-action — destructive choices are never remembered). Cookie amendment: D38's "never persisted" referred to contents; the SOURCE is a preference. | alpha.9 user feedback |
| D87 | Archive entries are deletable | The app-owned archive (D75) could be created, read (reveal/export/sync) but never pruned from the ui — the only escape was hand-editing the file. History rows now carry a ✕ ("remove from archive"): an in-app confirm states exactly what happens (re-queueing downloads again; the history entry stays; the file on disk is untouched) and then removes the `(extractor, vid)` line with an atomic rewrite that preserves every other line verbatim — comments, blank lines, append-order, duplicates of other entries. Matching mirrors archive_contains (extractor case-insensitive, vid exact); a no-match removal is a no-op, reported honestly. History rows are deliberately NOT deleted by this — they record that a download happened; deleting history is a separate concern. | user request |
| D88 | Retry means the current composer, not the job's frozen options | The history row's ↻ (re-download) and the queue row's retry used the options captured at queue time — turn on cookies or change the format and retry silently ignored it, an outcome no user would expect. Retry now re-queues with the **live composer state** (the same D19 mirror the preview uses): destination changes move the download, and the old folder's partial artifacts are swept at finalize (finished files are never touched). The one deliberate exception is `overwrite` — it stays per-action (the dialog grant / history ↻), never remembered, so "replace the file" is always an explicit choice. | user request |
| D89 | Thumbnail embedding is format-truthful | Live-probed with real yt-dlp: only **wav** (audio) and **webm** (video) containers cannot hold embedded cover art — every other format accepts it. The engine used to pass `--embed-thumbnail` unconditionally, which errored the whole job for wav and scattered orphaned `.webp`/`.png` sidecars. Now: wav skips the flag, a finalize-time sweep removes orphaned sidecars when a real target exists, and the composer shows a "cover art will be skipped" note for wav/webm. | user request |
| D90 | Webm selection is container-aware | The 1080p "best" format map picked the highest-bitrate **m4a/aac** track, which cannot live in a webm container — every such job died with ffmpeg "Conversion failed!". Webm is a strict container (VP8/VP9/AV1 video + Vorbis/Opus audio only). The engine now selects webm-native streams for webm (falling back down the chain), and errors honestly instead of queueing a doomed merge. Found live: 4K "best" happened to pick webm-native audio, which is why the bug hid at 4K and bit at 1080p. | user request |
| D91 | E2E harness: per-group sandboxes, no cross-scenario state | The e2e suite's entire flake class was shared state — one scenario's leftover rows/files/composer options leaking into the next. The harness now runs each scenario **group** on a fresh data dir + destination (per-group `YTDLP_GUI_DATA_DIR`, wiped and re-seeded per group), with a fresh app launch per group. Cross-group leftovers are impossible by construction; in-group chains (e.g. S6's stopped row owning its URL under D33) are handled explicitly by the scenario. The redesign also surfaced the D34 gate bug: requiring *both* big jobs to write bytes can never fire when one job's real transfer (4 s) ends before the other's metadata phase (13 s) begins — the gate now keys on the first writer while the second holds the concurrency slot. | engineering |
...
1. **add downloads card** (always visible, inline — no pop-up, D11):
   ...
   - **advanced ▾** disclosure: cookies (D38: none / from browser / cookies.txt
     — persisted as a preference since D86), subtitles (D39), sponsorblock (D39), extra args
...
- Cookies: exported browser session cookies are **sensitive** — held in memory
  per job only — the app never stores cookie contents; the SOURCE choice
  persists as a composer preference (D86).
