...
| 21 | archive↔db reconciliation (D64) | history → "sync history from archive" after appending a fake `e2efake <id>` line to the app archive | report shows +1 row backfilled ("source url unknown"); a second reconcile backfills nothing (idempotent); no archive line or history row is ever pruned |
| 22 | reset app data (D69) | settings → app → "reset app data…", confirm twice with downloads running and again when idle | refused with a busy message while anything runs; when idle: queue, history, settings.json and the app-owned downloaded.txt are gone from the profile, bin/ survives, and the report matches what the disk shows |
| 23 | archive import merge/replace (D75) | history → "import archive…" on a foreign txt (ids A,B), re-import after the app archive gains a separate id C, then again choosing replace | merge unions (archive holds A,B then A,B,C; the picked file is never modified); replace swaps the archive to exactly the imported content while history keeps C's row (never deleted); the default mode is merge; re-import reports 0 new; settings.archivePath stays empty |
